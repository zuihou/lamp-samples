package top.tangyh.lamp.noneMultipleDataSources.config.datasource;
...
@Slf4j
@MapperScan(
        basePackages = {"top.tangyh.lamp.noneMultipleDataSources.dao.slave"}, annotationClass = Repository.class,
        sqlSessionFactoryRef = SlaveDatabaseAutoConfiguration.DATABASE_PREFIX + "SqlSessionFactory")
@EnableConfigurationProperties({MybatisPlusSlaveProperties.class})
@ConditionalOnExpression("!'DATASOURCE'.equals('${lamp.database.multiTenantType}')")
public class SlaveDatabaseAutoConfiguration extends BaseDatabaseConfiguration {
    ...
    public SlaveDatabaseAutoConfiguration(MybatisPlusSlaveProperties properties,
